...
QualityMetrics calculate_quality_metrics(const voxblox::LongIndexSet& gt_occ_all, const voxblox::LongIndexSet& gt_obs_occ,
                               const voxblox::LongIndexSet& gt_free_all, const voxblox::LongIndexSet& gt_obs_free,
                               const voxblox::LongIndexSet& map_obs_occ, const voxblox::LongIndexSet& map_obs_free) {
    // calculate counts
    ...
    metrics.precision_overall = map_obs_correct / set_union(map_obs_occ, map_obs_free);
...
CoverageMetrics calculate_coverage_metrics(const voxblox::LongIndexSet& gt_occ_all, const voxblox::LongIndexSet& gt_obs_occ,
                                const voxblox::LongIndexSet& gt_free_all, const voxblox::LongIndexSet& gt_obs_free,
                                const voxblox::LongIndexSet& map_obs_occ, const voxblox::LongIndexSet& map_obs_free) {
...
void fill_dummy_data(std::shared_ptr<voxblox::Layer<voxblox::TsdfVoxel>> ground_truth_layer,
...

    {
        ...
        for (size_t x = 0; x < 2 * ground_truth_layer->voxels_per_side(); x++) {
            for (size_t y = 0; y < ground_truth_layer->voxels_per_side(); y++) {
                ...
                voxblox::TsdfVoxel* voxel = ground_truth_layer->getVoxelPtrByGlobalIndex(voxelIdx);
                voxel->weight = 1;
                voxel->distance = 100;
            }
        }
        ...
        for (size_t x = 0; x < observed_layer->voxels_per_side(); x++) {
            for (size_t y = 0; y < observed_layer->voxels_per_side(); y++) {
                ...
                voxblox::TsdfVoxel* voxel = observed_layer->getVoxelPtrByGlobalIndex(voxelIdx);
                voxel->weight = 1;
                voxel->distance = 100;
            }
        }
...
void test_eval_metrics() {
    ...
    calculate_Intersection_difference(*ground_truth_layer, *observed_layer, &intersection_gt, &difference_gt);
