...
voxblox::GlobalIndex get_voxel_index_from_point(
    const voxblox::Point& point, voxblox::FloatingPoint voxel_size_inv)  {
  return voxblox::getGridIndexFromPoint<voxblox::GlobalIndex>(point, voxel_size_inv);
}

// returns the state of a voxel, wheter occupied, free or unknown
int get_voxel_state(
    const voxblox::GlobalIndex& index, const voxblox::Layer<voxblox::TsdfVoxel>& layer) {
...
void compute_free_occupied_space_frontier(
    const voxblox::Layer<voxblox::TsdfVoxel>& layer, const voxblox::Point& initial_point, voxblox::GlobalIndexVector * voxels, voxblox::GlobalIndexVector * obstacles) {
...
void get_free_and_occupied_voxels_from_layer(const voxblox::Layer<VoxelType>& layer,
    ...
    for (const voxblox::BlockIndex& block_idx : blocks) {
        const voxblox::Block<VoxelType>& block = layer.getBlockByIndex(block_idx);

        for (size_t linear_index = 0; linear_index < num_voxels_per_block; ++linear_index) {
            // voxblox::Point coord = block.computeCoordinatesFromLinearIndex(linear_index);
            auto voxel = block.getVoxelByLinearIndex(linear_index);
            voxblox::VoxelIndex voxel_idx = block.computeVoxelIndexFromLinearIndex(linear_index);
            voxblox::GlobalIndex global_voxel_idx = voxblox::getGlobalVoxelIndexFromBlockAndVoxelIndex(block_idx, voxel_idx, layer.voxels_per_side());

            if (voxblox::utils::isObservedVoxel(voxel)) {
                bool is_occupied = voxblox::utils::isOccupied(voxel, layer.voxel_size());
...
void refine_gt_layer(const voxblox::Layer<voxblox::TsdfVoxel>& gt_layer,
                     voxblox::Layer<voxblox::TsdfVoxel>::Ptr refined_layer, const voxblox::Point start = voxblox::Point(0.0, 0.0, 0.0)) {
    voxblox::GlobalIndexVector gt_occ_voxels, gt_free_voxels;
    compute_free_occupied_space_frontier(gt_layer, start, &gt_free_voxels, &gt_occ_voxels);
...
void fill_gt_layer(voxblox::Layer<voxblox::TsdfVoxel>::Ptr  gt_layer, const voxblox::Point start = voxblox::Point(0.0, 0.0, 0.0)) {
    voxblox::GlobalIndexVector gt_occ_voxels, gt_free_voxels;
    compute_free_occupied_space_frontier(*gt_layer, start, &gt_free_voxels, &gt_occ_voxels);

    for (auto global_voxel_idx : gt_free_voxels) {
       voxblox::TsdfVoxel* free_voxel = gt_layer->getVoxelPtrByGlobalIndex(global_voxel_idx);

        // check if the block containing the voxel exists.
        if (free_voxel == nullptr) {
            // ssc_map_->getSSCLayerPtr()->a
            auto block_idx = voxblox::getBlockIndexFromGlobalVoxelIndex(global_voxel_idx, gt_layer->voxels_per_side_inv());
            auto block = gt_layer->allocateBlockPtrByIndex(block_idx);
            const voxblox::VoxelIndex local_voxel_idx =
                voxblox::getLocalFromGlobalVoxelIndex(global_voxel_idx, gt_layer->voxels_per_side());
            free_voxel = &block->getVoxelByVoxelIndex(local_voxel_idx);
        }
        free_voxel->weight = 1.0f;
        free_voxel->distance = 100.0f;//refined_layer->voxel_size();
...
void calculate_Intersection_difference(const voxblox::Layer<VoxelTypeA>& layer,
                                       const voxblox::Layer<VoxelTypeB>& otherLayer, voxblox::GlobalIndexVector* intersection,
    ...
    for (const voxblox::BlockIndex& index : blocks) {
        ...
        for (size_t linear_index = 0; linear_index < num_voxels_per_block; ++linear_index) {
            ...
            if (voxblox::utils::isOccupied(gt_voxel, layer.voxel_size())) {
                ...
                if (observed_voxel != nullptr && voxblox::utils::isOccupied(*observed_voxel, otherLayer.voxel_size())) {
...
template <typename VoxelType>
void prune_inside_voxels(const voxblox::Layer<VoxelType>& layer, voxblox::GlobalIndexVector* voxels) {
...
template <typename VoxelType>
void prune_outlier_voxels(const voxblox::Layer<VoxelType>& layer, voxblox::GlobalIndexVector* voxels) {
    ...
    for (const voxblox::BlockIndex& index : blocks) {
        ...
        for (size_t linear_index = 0; linear_index < num_voxels_per_block; ++linear_index) {
            ...
            voxblox::GlobalIndex global_voxel_idx =
                voxblox::getGlobalVoxelIndexFromBlockAndVoxelIndex(block_idx, voxel_idx, layer.voxels_per_side());
            auto voxel_coords = voxblox::getCenterPointFromGridIndex(global_voxel_idx, layer.voxel_size());
...
template <typename VoxelType>
void split_observed_unobserved_voxels(const voxblox::Layer<VoxelType>& layer, const voxblox::LongIndexSet& in_voxels, voxblox::LongIndexSet* out_observed_voxels,  voxblox::LongIndexSet* out_un_observed_voxels ) {
